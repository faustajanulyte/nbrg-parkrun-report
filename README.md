...
In a container the browser runs in the background and cannot ask the user to complete a CAPTCHA, so a challenged report stops with a clear error. The first run can take a few minutes, and the interface displays a loading status throughout. Athlete histories are cached locally for 12 hours, making repeat checks faster.
